...
Метод генерирует коды капчи, - отображаемый и секретный, - сохраняет их в базу данных (таблица ```captcha_codes```) и возвращает секретный код ```secret``` (поле в базе данных```secret_code```) и картинку с отображённым на ней основным кодом капчи ```image``` (поле базе данных ```code```).
